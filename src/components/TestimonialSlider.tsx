...
// Update the getVisibleTestimonials function to handle proper index calculation
const getVisibleTestimonials = (currentIndex: number, totalItems: number) => {
  const indices = [];
  // Get current and next two indices with proper wrapping
  for (let i = 0; i < 3; i++) {
...
// Update the particles state type
interface Particle {
  left: string;
  top: string;
  delay: string;
...
export default function EnhancedTestimonialSlider() {
  ...
  // Update the useEffect for faster auto-scroll
  useEffect(() => {
    if (!isAutoPlay) return;

    const interval = setInterval(() => {
      paginate(1);
    }, 2000); // Changed from 4000 to 2000 for faster transitions

    return () => clearInterval(interval);
  }, [paginate, isAutoPlay]);

  // Add a function to toggle autoplay
  ...
  return (
    <section className="py-12 md:py-24 px-4 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        {/* Updated Navigation and Testimonials Container */}
        <div className="relative max-w-6xl mx-auto mb-10 md:mb-20">
          ...
          {/* Updated Testimonials Container */}
...
// Update TestimonialCard for better responsiveness
...
// Add these icon components at the top with your other icons
